...
<button
onClick={toggleTheme}
className={absolute top-6 right-6 p-3 rounded-full ${ isDark ? 'bg-yellow-400 text-black hover:bg-yellow-300' : 'bg-gray-800 text-yellow-400 hover:bg-gray-700' }}
>
{isDark ? '☀️' : '🌙'}
</button>
...
Copy
Edit
    <div className={`p-10 rounded-2xl shadow-2xl w-96 ${
      isDark ? 'bg-gray-800 text-white border border-gray-700' : 'bg-white text-gray-900'
    }`}>
      <h1 className={`text-4xl font-bold text-center mb-2 ${isDark ? 'text-blue-400' : 'text-blue-600'}`}>
        Welcome Back!
      </h1>
      <p className={`text-md text-center mb-6 ${isDark ? 'text-gray-300' : 'text-gray-600'}`}>
